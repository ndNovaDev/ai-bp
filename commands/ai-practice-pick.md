...
写之前必须给用户一次开口的机会。Claude 不会读心 —— 光证据 + 四条方向能拼出"做了什么",拼不出"用户想突出哪条线 / 想保留或删什么"。哪怕用户只给一句"重点要 X",都比模型凭空猜强一大截。

每个 topic 一次。两小步:

**5.1 — Claude 先在主对话里草一段全文简述**(2-4 句),写清楚:这个 topic 你准备从哪儿切入、要讲哪几件事、结尾落到哪儿、读者读完应该带走什么。这段不是最终文章,只是"方向草案"给用户拍板用。
...
1. **label**: "采纳 (推荐)" — **description**: "就按上面这段方向写"
2. **label**: "我自己来" — **description**: "在 Other 里填:大纲条目 / 重点 X / 删 Y 段 / 按 1-2-3 顺序 / 我担心 Y 没讲清楚 / 你自己的简述 / 任何方向 —— 都行"

处理规则:

- 选 1 → 进步 6,Claude 自己的简述作为主线
- 选 2 + Other 有内容 → Other 内容作为主线,优先级高于四条方向
- 选 2 + Other 为空 → 当作选 1 处理(回退到 Claude 自己的简述)
