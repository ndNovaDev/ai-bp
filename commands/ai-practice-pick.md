...
按下面四条方向直接写出来。这是第一版,留到步 6 打散后才是最终稿。
...
步 5 第一版必然带 LLM 默认节奏。这一步把它打散。

逐子句重写。每个逗号 / 句号 / 分号 / 冒号之间的串,**都强制换一遍** —— 随机长度,随机句式,继续按王小波口吻。

**唯一硬性要求:通顺**。不为了打散把意思搞坏,不把句子写得不像中文。除此之外不设任何约束 —— 不列变换维度,不预设 SOP。
...
多 topic:一篇 markdown,H1 是期号,每 topic 一个 H2。所有 topic 都过完步 5 + 步 6 之后再一次性落盘。
