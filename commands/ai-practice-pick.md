...
重排完段落骨架应该是**头重脚轻**(结论压顶,后面铺关键判断 + 1-2 个细节),不是均匀铺陈。

## 步 8 — 句式打散(必做)

步 7 后段落对了,句子节奏还是 LLM 默认的。这一步把它打散。
...
多 topic:一篇 markdown,H1 是期号,每 topic 一个 H2。所有 topic 都过完步 5 + 步 6 + 步 7 + 步 8 之后再一次性落盘。
