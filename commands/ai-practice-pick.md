...
一道 `AskUserQuestion`,二选一:

- "我自己写大纲 / 给主张和想法(下一条消息发) (强烈推荐)"
- "让 Claude 基于证据先草一份"

**为什么强烈推荐你自己来**:Claude 不会读心术。它能从步 4 的 git / file / jsonl 拼出"你做了什么",
但**不知道**你想突出哪条线、哪点你最得意、想给读者什么 take-away。自动起草的大纲多半不是你
心里那张图 — 落盘之后你只会觉得"嗯还行但不太对",白折腾。给你自己 30 秒写 3-5 条 bullet,
比 Claude 猜半天更值。

可以给的形式(怎么轻松怎么来):
- 一份你的大纲(一行一段主题句,5-12 行最好)
- 几个 bullet(你的主张、想说的话、想保留的细节)
- 一句"重点是 X"的指令 + 几个不能漏的事实

**如果用户选"我自己写"**:你回 "好,把大纲(或者你的想法、几个 bullet)发我,主张越具体我越能照着你的写。"
**当前 slash command 流程结束**。用户下条消息发来之后,主对话里你看到自然续上 5d
(把用户给的 bullet/主张扩写成段落,跳过 5c)。

**如果用户选"让 Claude 先草"**:基于(步 4 证据 + 5a 答复),在下一条回复里产出大纲。
一行一段主题句,**5-12 行**(案例长度决定;短就 5,长就 12,不必凑数)。形状(fenced 贴出来):
...
#### 5c — Claude 起草大纲后的确认(仅 5b 选了 "让 Claude 先草" 才跑)

一道 `AskUserQuestion`,二选一:

- "采纳 (推荐)" → 进 5d
- "微调某条 (Other 填修改指令)" → 应用修改后进 5d

不必再请用户验证修改后的大纲 — 已经够多 round-trip 了。

(如果用户在 5b 选了"我自己写",这步跳过,直接进 5d。)
...
#### 5g — 反推大纲 sanity check(必须通过)

从修剪后的版本反向提取每段主题句,组成"事后大纲"。跟用户/Claude 起草的大纲对比:

- 几乎一致 → 通过,进 5h 落盘。
- 差异大但事后大纲更连贯 → 通过(说明写的过程中真想清楚了,这是 Peterson 流程的预期效果)。
- 差异大且事后大纲松散 → **回 5e/5f 重新组织段落,循环到通过**。一次过把事情做对 —
  落盘的就是最终版本,不要把质量问题留到落盘后让用户 review。AI 不会读心术,用户也不会逐字校对。
...
**这就是最终版本,不是草稿**。落盘前 5d-5g 应该已经把质量问题全部解决 ——
段落写完整 / 砍句砍段做扎实 / 反推大纲 sanity 通过。
