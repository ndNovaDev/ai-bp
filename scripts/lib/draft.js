...
//
// 当前方案在 commands/ai-practice-pick.md 步 5 + 步 6 + 步 7:
//   步 5 采访  :Claude 先草一段全文简述,然后 AskUserQuestion 二选一:
//                "采纳"(用 Claude 自己的简述当主线) / "我自己来"(用户在 Other 里填自己的方向)。
//                注:options 是决策,不是输入形态分类 —— 0.1.37 把选项写成
//                "大纲/简述/要点"分类,用户合理地以为选了某项就会得到那类东西,
//                结果选了之后什么都没发生,UX 大坑。0.1.38 改成"采纳/我自己来"。
//   步 6 第一版:四条正向方向(王小波口吻 / 金字塔原理 / 知识诅咒 guard / 读者画像)。
//                用户给了方向就按用户方向走,四条方向退居兜底。
//   步 7 打散  :逐子句重写,长短随机 + 句式随机 + 王小波口吻,唯一硬约束是通顺。
//                不列变换维度,不预设 SOP。
